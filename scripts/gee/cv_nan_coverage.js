// Google Earth Engine script to assess per-band, per-season coverage (non-NaN / unmasked)
// for Cape Verde islands using the same timeframes and a similar cloud mask
// as the local pipeline. Layers highlight pixels with valid data in red.
//
// How to use:
// - Paste this script into https://code.earthengine.google.com/
// - It adds console summaries and map layers (hidden by default to protect the browser).
// - Toggle layers on as needed. Coverage stats are printed in the Console.
//
// Safety/perf:
// - Visualization is coarse when possible; analysis uses scale=10 m.
// - tileScale is increased for reduceRegion calls to avoid client timeouts.

// -------------------------------
// 1) Config (match local pipeline)
// -------------------------------
var TIMESTAMPS = [
...
var CLOUDY_PIXEL_PERCENTAGE = 100; // keep consistent with local config

// Bands to test from GEE (Sentinel-2 SR). These are NOT used by the pipeline today,
// but are candidates to add. The script checks coverage for each.
// - B5,B6,B7: red-edge bands (vegetation structure/chlorophyll)
// - B8A: narrow NIR (complements B8)
// - B1: coastal aerosol (can be noisy; listed for completeness)
// - B9: water vapor (atmospheric; rarely used as predictor)
// - B10: cirrus (mainly for QC/masking)
var S2_CANDIDATE_BANDS = ['B5','B6','B7','B8A','B1','B9','B10'];

// Sentinel-1 GRD polarizations to test per timeframe (VV/VH)
var S1_POLARIZATIONS = ['VV','VH'];

// DEM source to test (constant in time). We also derive slope/aspect.
// Use NASADEM (public) to avoid access issues with COPERNICUS/DEM/GLO-30.
// If you have access to COP DEM, swap to:
//   ee.ImageCollection('COPERNICUS/DEM/GLO-30').mosaic()
var DEM = ee.Image('NASA/NASADEM_HGT/001'); // band: 'elevation'

// Visualization (keep layers lightweight by default)
var SHOW_LAYERS = false;       // add layers hidden by default to protect the browser
var VIS_SCALE_DISPLAY = 60;    // display scale in meters (analysis uses 10 m)
var LAYER_OPACITY = 1.0;       // red, 100% opacity as requested

// -----------------------------------
// 2) ROI: Cape Verde islands polygons
//     Copy from scripts/a1_phase1_data_download.py (commented `islands` list)
// -----------------------------------
// IMPORTANT: The coordinates below mirror the commented block in a1_phase1_data_download.py.
// If that block changes, paste the updated arrays here.
var islands = [
  { name: 'Boa_Vista', coords: [
...
var fc = ee.FeatureCollection(islands.map(function (o) {
  return ee.Feature(ee.Geometry.Polygon([o.coords]), {name: o.name});
}));
var ROI = fc.geometry();

// Center map without invoking Geometry.centroid (to avoid ErrorMargin type issues)
// Cape Verde rough center; adjust zoom as needed
Map.setCenter(-24.5, 16.0, 8);

// -------------------------------
// 3) Helpers
// -------------------------------
function cloudMaskS2sr(img) {
  var qa = img.select('QA60');
  var cloudBit = 1 << 10;   // cloud
  var cirrusBit = 1 << 11;  // cirrus
  var keep = qa.bitwiseAnd(cloudBit).eq(0).and(qa.bitwiseAnd(cirrusBit).eq(0));
  return img.updateMask(keep);
}

function covStatsFromMask(maskImg, label, scale) {
  // maskImg: 1 = has data, 0 = masked
  var stats = maskImg.reduceRegion({
  ...
  var frac = ee.Number(stats.get(maskImg.bandNames().get(0))); // mean of 0/1 mask
  var pct = ee.Number(frac).multiply(100);
  print(label + ' | coverage %:', pct);
  return pct;
}

function addMaskLayer(maskImg, name) {
  if (!SHOW_LAYERS) return;
  // Show only non-NaN pixels in red, 100% opacity.
  var vis = {min: 0, max: 1, palette: ['000000','ff0000'], opacity: LAYER_OPACITY};
  Map.addLayer(maskImg.reproject({crs: maskImg.projection(), scale: VIS_SCALE_DISPLAY}), vis, name, false);
}

// Convert a band into a 0/1 has-data mask from an image (band may be missing)
function hasDataMask(img, band) {
  // Assume band exists in the median image; when missing, select would error.
  // For robustness, rely on median over SR/GRD collections including the band,
  // and treat missing bands as zero coverage upstream.
  var m = img.select([band]).mask();
  return m.rename('hasData').unmask(0).gt(0).toInt();
}

// Safe divide helper to avoid NaNs from zero denominators
function safeDivide(num, den) {
  var d = den.where(den.eq(0), 1);
  return num.divide(d);
}

// -------------------------------
// 4) Sentinel-2 coverage per season
// -------------------------------
print('--- Sentinel-2 SR candidate bands coverage ---');
TIMESTAMPS.forEach(function (win, sIdx) {
  var start = win[0], end = win[1];
  var col = ee.ImageCollection('COPERNICUS/S2_SR_HARMONIZED')
      .filterBounds(ROI)
      .filterDate(start, end)
      .filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE', CLOUDY_PIXEL_PERCENTAGE))
      .map(cloudMaskS2sr);
  var med = ee.ImageCollection(col).median();

  S2_CANDIDATE_BANDS.forEach(function (b) {
    var mask = hasDataMask(med, b);
    addMaskLayer(mask, 'S2 ' + b + ' s' + (sIdx+1));
    covStatsFromMask(mask, 'S2 ' + b + ' s' + (sIdx+1), 10);
  });
});

// -------------------------------
// 4b) Sentinel-2 derived indices coverage per season
// -------------------------------
print('--- Sentinel-2 derived indices coverage ---');
TIMESTAMPS.forEach(function (win, sIdx) {
  var start = win[0], end = win[1];
  var col = ee.ImageCollection('COPERNICUS/S2_SR_HARMONIZED')
      .filterBounds(ROI)
  ...
  var green = med.select('B3');
  var red   = med.select('B4');
  var nir   = med.select('B8');
  var re1   = med.select('B5');
  var swir1 = med.select('B11');

  // NDVI
  var ndvi_den = nir.add(red);
  var ndvi = safeDivide(nir.subtract(red), ndvi_den).rename('NDVI');
  var ndviMask = ndvi.mask().unmask(0).gt(0).toInt();
  addMaskLayer(ndviMask, 'IDX NDVI s' + (sIdx+1));
  covStatsFromMask(ndviMask, 'IDX NDVI s' + (sIdx+1), 10);

  // NDRE (red-edge NDVI): (NIR - B5) / (NIR + B5)
  var ndre_den = nir.add(re1);
  var ndre = safeDivide(nir.subtract(re1), ndre_den).rename('NDRE');
  var ndreMask = ndre.mask().unmask(0).gt(0).toInt();
  addMaskLayer(ndreMask, 'IDX NDRE s' + (sIdx+1));
  covStatsFromMask(ndreMask, 'IDX NDRE s' + (sIdx+1), 10);

  // GNDVI: (NIR - Green) / (NIR + Green)
  var gndvi_den = nir.add(green);
  var gndvi = safeDivide(nir.subtract(green), gndvi_den).rename('GNDVI');
  var gndviMask = gndvi.mask().unmask(0).gt(0).toInt();
  addMaskLayer(gndviMask, 'IDX GNDVI s' + (sIdx+1));
  covStatsFromMask(gndviMask, 'IDX GNDVI s' + (sIdx+1), 10);

  // OSAVI: (1+0.16)*(NIR-Red)/(NIR+Red+0.16)
  var osavi_den = nir.add(red).add(0.16);
  var osavi = safeDivide(nir.subtract(red).multiply(1.16), osavi_den).rename('OSAVI');
  var osaviMask = osavi.mask().unmask(0).gt(0).toInt();
  addMaskLayer(osaviMask, 'IDX OSAVI s' + (sIdx+1));
  covStatsFromMask(osaviMask, 'IDX OSAVI s' + (sIdx+1), 10);

  // NDWI (McFeeters): (Green - NIR) / (Green + NIR)
  var ndwi_den = green.add(nir);
  var ndwi = safeDivide(green.subtract(nir), ndwi_den).rename('NDWI');
  var ndwiMask = ndwi.mask().unmask(0).gt(0).toInt();
  addMaskLayer(ndwiMask, 'IDX NDWI s' + (sIdx+1));
  covStatsFromMask(ndwiMask, 'IDX NDWI s' + (sIdx+1), 10);

  // BSI: (SWIR + Red - NIR - Blue) / (SWIR + Red + NIR + Blue)
  var bsi_num = swir1.add(red).subtract(nir).subtract(blue);
  var bsi_den = swir1.add(red).add(nir).add(blue);
  var bsi = safeDivide(bsi_num, bsi_den).rename('BSI');
  var bsiMask = bsi.mask().unmask(0).gt(0).toInt();
  addMaskLayer(bsiMask, 'IDX BSI s' + (sIdx+1));
  covStatsFromMask(bsiMask, 'IDX BSI s' + (sIdx+1), 10);
});

// -------------------------------
// 5) Sentinel-1 coverage per season (VV/VH)
// -------------------------------
print('--- Sentinel-1 GRD (VV/VH) coverage ---');
TIMESTAMPS.forEach(function (win, sIdx) {
  var start = win[0], end = win[1];
  var col = ee.ImageCollection('COPERNICUS/S1_GRD')
      ...
      // Keep both ascending and descending passes
      .filter(ee.Filter.inList('orbitProperties_pass', ['ASCENDING', 'DESCENDING']))
      .map(function (img) { return img.updateMask(img.mask()); });
  var med = ee.ImageCollection(col).median();
  S1_POLARIZATIONS.forEach(function (pol) {
    var mask = hasDataMask(med, pol);
    addMaskLayer(mask, 'S1 ' + pol + ' s' + (sIdx+1));
    covStatsFromMask(mask, 'S1 ' + pol + ' s' + (sIdx+1), 10);
  });
});

// -------------------------------
// 6) DEM + derived slope/aspect coverage (constant)
// -------------------------------
print('--- DEM (GLO-30) coverage ---');
var demMask = DEM.mask().rename('hasData').unmask(0).gt(0).toInt();
addMaskLayer(demMask, 'DEM elevation');
covStatsFromMask(demMask, 'DEM elevation', 30);

var slope = ee.Terrain.slope(DEM);
var aspect = ee.Terrain.aspect(DEM);
var slopeMask = slope.mask().unmask(0).gt(0).toInt();
var aspectMask = aspect.mask().unmask(0).gt(0).toInt();
addMaskLayer(slopeMask, 'DEM slope');
addMaskLayer(aspectMask, 'DEM aspect');
covStatsFromMask(slopeMask, 'DEM slope', 30);
covStatsFromMask(aspectMask, 'DEM aspect', 30);

print('Done. Toggle layers on as needed. Red = valid (non-NaN) pixels.');
